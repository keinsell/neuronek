		...
		return new Substance(
			{
				...
				administrationRoutes: entity.routesOfAdministraton.map(
					(roa) => routeOfAdministrationMapper.toDomain(roa),
				),
				effects: entity.OccuranceOfEffect
					? entity.OccuranceOfEffect.map(
							(occurance) => new EffectOccuranceMapper().toDomain(occurance),
					  )
					: [],
			},
			entity.id,
