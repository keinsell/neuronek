...
export const Username = new t.Type<string>( 'Username', (input : unknown) : input is string => {
	return (typeof input === 'string' && new UsernameIsAboveMinimalCharacterLimit().satisfy( input ) && new UsernameIsBelowMaxiumCharacterLimit().satisfy( input ))
}, (input, context) => {
...
export async function createUsername(username : string) : Promise<Username> {
	...
	if(!new UsernameIsAboveMinimalCharacterLimit().satisfy( valid ) || !new UsernameIsBelowMaxiumCharacterLimit().satisfy( valid )) {
		throw new InvalidValue( `Username must be between ${ UsernameIsAboveMinimalCharacterLimit.MINIMAL_LENGTH } and ${ UsernameIsBelowMaxiumCharacterLimit.MAXIMAL_LENGTH } characters long` )
	}
