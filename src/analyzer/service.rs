...
use crate::analyzer::model::Report;
...
) -> miette::Result<Option<IngestionReport>>
{
    let substance = get_substance(&analyze_ingestion.substance)
        .await
    ...
    if substance.is_some()
    {
        ...
        return Ok(Some(analysis_report));
    }
