...
CREATE TABLE `seaql_migrations` (`version` varchar NOT NULL, `applied_at` bigint NOT NULL, PRIMARY KEY (`version`));
-- Create "substance" table
CREATE TABLE `substance` (`id` text NOT NULL, `name` text NOT NULL, `common_names` text NOT NULL, `pubchem_cid` integer NOT NULL, `psychonautwiki_url` text NULL, `psychoactive_class` text NOT NULL, `chemical_class` text NULL, `description` text NULL, PRIMARY KEY (`id`));
...
CREATE TABLE `substance_route_of_administration_dosage` (`id` text NOT NULL, `intensity` text NOT NULL, `lower_bound_amount` real NULL, `upper_bound_amount` real NULL, `unit` text NOT NULL, `routeOfAdministrationId` text NULL, PRIMARY KEY (`id`), CONSTRAINT `route_of_administration_dosage_routeOfAdministrationId_fkey` FOREIGN KEY (`routeOfAdministrationId`) REFERENCES `substance_route_of_administration` (`id`) ON UPDATE CASCADE ON DELETE SET NULL);
...
CREATE TABLE `formulation` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `name` text NOT NULL, `summary` text NULL, `labeller` text NULL, `form` text NULL, `route` text NOT NULL, CHECK (
        route IN (
                  'buccal',
                  'inhaled',
                  'insufflated',
...
CREATE TABLE `formulation_ingredient` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `formulation_name` integer NOT NULL, `substance_name` text NOT NULL, `dosage` real NOT NULL, CONSTRAINT `0` FOREIGN KEY (`formulation_name`) REFERENCES `formulation` (`name`) ON UPDATE CASCADE ON DELETE CASCADE);
