...
use atty::Stream;
use clap::{arg, command, ColorChoice, CommandFactory, Parser, Subcommand, ValueEnum, Args};
...
pub enum CompletionSubcommands {
    ...
    #[command(
        ...
        long_about = "Generate shell completion scripts for neuronek.\\n\\nExamples:\\n  # Bash\\n  neuronek completion generate --shell bash > ~/.local/share/bash-completion/completions/neuronek\\n  \\n  # Zsh\\n  neuronek completion generate --shell zsh > ~/.zfunc/_neuronek\\n  \\n  # Fish\\n  neuronek completion generate --shell fish > ~/.config/fish/completions/neuronek.fish\\n  \\n  # PowerShell\\n  neuronek completion generate --shell powershell | Out-String | Invoke-Expression\\n  \\n  # Nushell\\n  neuronek completion generate --shell nushell | save ~/.config/nushell/completions/neuronek.nu"
    )]
    Generate {
        #[arg(value_enum)]
...
impl Displayable for CompletionScriptOutput {
    fn as_pretty(&self) -> String {
        format!("Successfully generated {} completion script. Please follow your shell's instructions to install it.", self.shell_name)
    }
}
...
impl Displayable for CacheRefreshOutput {
    fn as_pretty(&self) -> String {
        if self.refreshed_caches.is_empty() {
            ...
            format!("Successfully refreshed cache(s): {}", self.refreshed_caches.join(", "))
        }
    }
}
...
impl Executable<()> for CompletionArgs {
    async fn execute(self, ctx: &Session) -> miette::Result<()> {
        match self.command {
            CompletionSubcommands::Generate { shell } => {
                ...
                match shell {
                    CompletionShell::Bash => {
                        crate::cli::completions::generate_completion(clap_complete::Shell::Bash, &mut cmd, &mut std::io::stdout());
                    }
                    CompletionShell::Elvish => {
                        clap_complete::generate(clap_complete::shells::Elvish, &mut cmd, bin_name, &mut std::io::stdout());
                    }
                    CompletionShell::Fish => {
                        crate::cli::completions::generate_completion(clap_complete::Shell::Fish, &mut cmd, &mut std::io::stdout());
                    }
                    CompletionShell::PowerShell => {
                        clap_complete::generate(clap_complete::shells::PowerShell, &mut cmd, bin_name, &mut std::io::stdout());
                    }
                    CompletionShell::Zsh => {
                        crate::cli::completions::generate_completion(clap_complete::Shell::Zsh, &mut cmd, &mut std::io::stdout());
                    }
                    CompletionShell::Nushell => {
                        clap_complete::generate(clap_complete_nushell::Nushell, &mut cmd, bin_name, &mut std::io::stdout());
                    }
                };
                // The script is printed directly to stdout by clap_complete::generate
                // So we return a SimpleMessageOutput that will not be printed by the main handler,
                // or we can make the handler smarter. For now, an empty message.
                Ok(())
            }
            CompletionSubcommands::Cache { cache_command } => {
                match cache_command {
                    CacheManagementCommand::RefreshCache => {
                        ...
                        substances = substances.into_iter()
                            .map(|s| s.to_lowercase())
                            .collect::<std::collections::HashSet<_>>()
                            .into_iter()
                            .collect::<Vec<_>>();
                        substances.sort();
                        substance_cache.refresh(&(), &substances)
                    ...
                    CacheManagementCommand::GetCached { completion_type } => {
                        ...
                        if !cache.is_valid()? {
                            ...
                            cache.refresh(&(), &items_to_cache).wrap_err_with(|| format!("Failed to auto-refresh {} cache", type_name))?;
                            if !cache.is_valid()? {
                                 return Err(miette!("Cache is still invalid after attempting refresh for {}", type_name));
                            }
                        }
                        let values = cache.read_all_lines().wrap_err_with(|| format!("Failed to read {} cache", type_name))?;
